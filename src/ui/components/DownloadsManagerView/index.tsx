import { Box, SearchInput, Select, Icon, Button, Pagination, PaginationProps, Scrollable } from '@Ektai.chat/fuselage';
import { useLocalStorage } from '@Ektai.chat/fuselage-hooks';
...
const DownloadsManagerView: FC = () => {
  ...
  const [searchFilter, setSearchFilter] = useLocalStorage('download-search', '');
  ...
  const serverFilterOptions = useSelector<RootState, [string, string][]>(({ downloads }) => [
  ...
  const handleServerFilterChange = useCallback((value: (typeof serverFilterOptions)[number][0]) => {
  ...
  const [mimeTypeFilter, setMimeTypeFilter] = useLocalStorage<(typeof mimeTypeOptions)[number][0]>('download-type', '');

  const handleMimeFilter = useCallback((value: (typeof mimeTypeOptions)[number][0]) => {
  ...
  const [statusFilter, setStatusFilter] = useLocalStorage<(typeof statusFilterOptions)[number][0]>('download-tab', DownloadStatus.ALL);

  const handleTabChange = useCallback((value: (typeof statusFilterOptions)[number][0]) => {
  ...
  const downloads = useSelector(({ downloads }: RootState) => {
    ...
    const searchPredicate: Predicate = searchFilter
      ...
      : () => true;
    const serverPredicate: Predicate = serverFilter !== '' && serverFilter !== '*'
      ? ({ serverUrl }) => serverUrl === serverFilter
      : () => true;
    const mimeTypePredicate: Predicate = mimeTypeFilter !== '' && mimeTypeFilter !== '*'
      ? ({ mimeType }) => mimeType?.split('/')?.[0] === mimeTypeFilter
      : () => true;
    const statusPredicate: Predicate = statusFilter !== '' && statusFilter !== DownloadStatus.ALL
  ...
  return <Box display={isVisible ? 'flex' : 'none'} flexDirection='column' height='100vh' backgroundColor='surface'>
    <Box
      minHeight={64}
      paddingInline={64}
      display='flex'
    ...
    >
      <Box is='div' color='default' fontScale='h1'>{t('downloads.title')}</Box>
    </Box>
    <Box display='flex' justifyContent='space-between' alignItems='center' marginBlock={8} paddingInline={64}>
      <Box display='flex' flexGrow={7} flexShrink={7} flexBasis='0' paddingInline={2}>
        <SearchInput
          value={searchFilter}
      ...
      <Box display='flex' flexGrow={3} flexShrink={3} flexBasis='0' paddingInline={2}>
        <Select
          value={serverFilter}
      ...
      <Box display='flex' flexGrow={1} flexShrink={1} paddingInline={2}>
        <Button small ghost title={t('downloads.filters.clear')} onClick={handleClearAll}>
    ...
    <Scrollable>
      <Box flexGrow={1} flexShrink={1} paddingBlock={8} paddingInline={64}>
        <Box>
          {downloads.slice(currentPagination, currentPagination + itemsPerPage)
            .map((downloadItem) => <DownloadItem key={downloadItem.itemId} {...downloadItem} />)}
        </Box>
      </Box>
    </Scrollable>
