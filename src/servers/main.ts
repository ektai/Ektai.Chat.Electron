...
export const resolveServerUrl = async (input: string): Promise<ServerUrlResolutionResult> => {
  ...
  try {
    ...
    if (!/(^https?:\/\/)|(\.)|(^([^:]+:[^@]+@)?localhost(:\d+)?$)/.test(input)) {
      return resolveServerUrl(`https://${ input }.Ektai.chat`);
    }
...
export const setupServers = async (localStorage: Record<string, string>): Promise<void> => {
  ...
  if (localStorage['Ektai.chat.hosts']) {
    try {
      const storedString = JSON.parse(localStorage['Ektai.chat.hosts']);
  ...
  if (localStorage['Ektai.chat.currentHost'] && localStorage['Ektai.chat.currentHost'] !== 'null') {
    currentServerUrl = localStorage['Ektai.chat.currentHost'];
  }

  servers = Array.from(serversMap.values());
  currentServerUrl = serversMap.get(currentServerUrl)?.url ?? servers[0]?.url ?? null;

  if (localStorage['Ektai.chat.sortOrder']) {
    try {
      const sorting = JSON.parse(localStorage['Ektai.chat.sortOrder']);
