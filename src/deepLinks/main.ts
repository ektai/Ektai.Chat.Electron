...
const isEktaiChatUrl = (parsedUrl: URL): boolean =>
  parsedUrl.protocol === 'Ektaichat:';

const isGoEktaiChatUrl = (parsedUrl: URL): boolean =>
  parsedUrl.protocol === 'https:' && parsedUrl.hostname === 'go.Ektai.chat';
...
const parseDeepLink = (input: string): { action: string, args: URLSearchParams } | null => {
  ...
  if (isEktaiChatUrl(url)) {
    const action = url.hostname;
    const args = url.searchParams;
    return { action, args };
  }

  if (isGoEktaiChatUrl(url)) {
