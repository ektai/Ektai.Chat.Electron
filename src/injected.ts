import { EktaiChatDesktopAPI } from './servers/preload/api';
...
const start = (): void => {
  ...
  const { Info: serverInfo = {} } = window.require('/app/utils/Ektaichat.info') ?? {};

  if (!serverInfo.version) {
    return;
  }

  window.EktaiChatDesktop.setServerInfo(serverInfo);
  ...
  window.EktaiChatDesktop.setUrlResolver(Meteor.absoluteUrl);

  Tracker.autorun(() => {
    const unread = Session.get('unread');
    window.EktaiChatDesktop.setBadge(unread);
  });

  Tracker.autorun(() => {
    const { url, defaultUrl } = settings.get('Assets_favicon') || {};
    window.EktaiChatDesktop.setFavicon(url || defaultUrl);
  });

  Tracker.autorun(() => {
    const { url, defaultUrl } = settings.get('Assets_background') || {};
    window.EktaiChatDesktop.setBackground(url || defaultUrl);
  });

  Tracker.autorun(() => {
    const siteName = settings.get('Site_Name');
    window.EktaiChatDesktop.setTitle(siteName);
  });
  ...
  Tracker.autorun(() => {
    ...
    window.EktaiChatDesktop.setUserPresenceDetection({
  ...
  window.Notification = class EktaiChatDesktopNotification extends EventTarget implements Notification {
    ...
    static requestPermission(): Promise<NotificationPermission> {
      return Promise.resolve(EktaiChatDesktopNotification.permission);
    }
    ...
    constructor(title: string, options: (NotificationOptions & { canReply?: boolean }) = {}) {
      ...
      this[destroyPromiseSymbol] = window.EktaiChatDesktop.createNotification({
        title,
        ...options,
        onEvent: this.handleEvent,
      }).then((id) => () => {
        window.EktaiChatDesktop.destroyNotification(id);
      });
